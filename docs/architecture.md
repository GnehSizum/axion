...
v0.1.13.0 builds on v0.1.12.0 security diagnostics by adding manifest capability profiles. Profiles such as `app-info`, `window-control`, `file-access`, and `dialog-access` expand to explicit commands, events, and protocols during manifest loading, while `doctor` and diagnostics reports expose both declared profiles and effective permissions. Axion public versions use four components to separate Servo baseline, Axion feature milestones, and bugfix releases; Cargo crates keep SemVer-compatible three-component versions. Signed permission manifests, automatic capability minimization, signed installers, auto-updates, broader native API coverage, restart fallback, devtools integration, and default cross-platform GUI CI remain later milestones.
