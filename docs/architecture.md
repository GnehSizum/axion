...
v0.1.8.0 completes the GUI smoke command and optional CI artifact milestone on top of the diagnostics model and local GUI smoke foundation completed through v0.1.7.0. The release adds `axion-cli gui-smoke`, captures Servo-backed GUI diagnostics reports to disk, stabilizes GUI smoke check identifiers, and provides an opt-in GitHub Actions GUI smoke job through `workflow_dispatch`. Axion public versions use four components to separate Servo baseline, Axion feature milestones, and bugfix releases; Cargo crates keep SemVer-compatible three-component versions. Installer generation, signing, auto-updates, broader native API coverage, and default cross-platform GUI CI remain later milestones.
