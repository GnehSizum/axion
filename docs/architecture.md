...
v0.1.14.0 builds on v0.1.13.0 capability profiles by adding `doctor` release-readiness summaries for development, bundle, and GUI smoke workflows. `doctor` and diagnostics reports now expose declared profiles, effective permissions, security gates, and readiness blockers in one pre-release view. Axion public versions use four components to separate Servo baseline, Axion feature milestones, and bugfix releases; Cargo crates keep SemVer-compatible three-component versions. Signed permission manifests, automatic capability minimization, signed installers, auto-updates, broader native API coverage, restart fallback, devtools integration, and default cross-platform GUI CI remain later milestones.
