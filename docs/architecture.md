...
v0.1.10.0 stabilizes the development loop on top of the development-server diagnostics completed through v0.1.9.0. The release clarifies `axion-cli dev` launch selection, packaged fallback behavior, managed frontend commands, per-window entry URLs, frontend asset watching with debounce/ignore rules, live window reload requests, and built-in lifecycle events including `window.ready`. Axion public versions use four components to separate Servo baseline, Axion feature milestones, and bugfix releases; Cargo crates keep SemVer-compatible three-component versions. Installer generation, signing, auto-updates, broader native API coverage, restart fallback, devtools integration, and default cross-platform GUI CI remain later milestones.
