...
v0.1.7.0 completes the diagnostics model and local GUI smoke milestone on top of the CI and diagnostics stabilization completed through v0.1.6.0. The release moves CLI diagnostics JSON onto an Axion-owned Rust report model, exposes the active report schema to frontends, aligns the bridge diagnostics demo with the shared top-level report fields, and adds `AXION_GUI_SMOKE=1` for local Servo-backed GUI regression checks with timeout control. Axion public versions use four components to separate Servo baseline, Axion feature milestones, and bugfix releases; Cargo crates keep SemVer-compatible three-component versions. Installer generation, signing, auto-updates, broader native API coverage, and CI GUI integration tests remain later milestones.
