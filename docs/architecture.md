...
v0.1.9.0 completes the development-server diagnostics milestone on top of the GUI smoke foundation completed through v0.1.8.0. The release clarifies `axion-cli dev` launch selection, packaged fallback behavior, managed frontend commands, per-window entry URLs, next-step diagnostics, frontend asset watching, report-only reload requests, and explicit reserved devtools behavior. Axion public versions use four components to separate Servo baseline, Axion feature milestones, and bugfix releases; Cargo crates keep SemVer-compatible three-component versions. Installer generation, signing, auto-updates, broader native API coverage, backend hot reload, devtools integration, and default cross-platform GUI CI remain later milestones.
