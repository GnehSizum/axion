...
v0.1.12.0 builds on the packaging improvements from v0.1.11.0 by tightening capability and security diagnostics. The release adds per-window security summaries, capability risk levels, command category counts, bridge protocol consistency checks, and remote-navigation warnings to `axion doctor`. Axion public versions use four components to separate Servo baseline, Axion feature milestones, and bugfix releases; Cargo crates keep SemVer-compatible three-component versions. Policy profiles, signed permission manifests, automatic capability minimization, signed installers, auto-updates, broader native API coverage, restart fallback, devtools integration, and default cross-platform GUI CI remain later milestones.
